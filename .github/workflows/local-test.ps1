...
Write-Host "Closing stale PRs in $repo, removing corresponding branches ..."
...
Write-Host "Scanning closed pull requests in $repo to delete branches ..."

# Get closed PRs using gh
$closedPrsJson = gh pr list --repo $repo --state closed --json "number,headRefName,mergedAt,author"
